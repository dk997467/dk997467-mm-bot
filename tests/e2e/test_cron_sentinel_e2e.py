...
def test_cron_sentinel_e2e(tmp_path):
    ...
    r = subprocess.run([sys.executable, '-m', 'tools.ops.cron_sentinel', '--window-hours', '24', '--artifacts-dir', str(artifacts), '--utc-today', today, '--out-json', str(out_json), '--out-md', str(out_md)], cwd=str(root), capture_output=True, text=True)
