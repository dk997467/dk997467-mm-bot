...
def test_make_ready_dry_and_real(tmp_path):
    ...
    r = subprocess.run([sys.executable, str(root / 'tools' / 'release' / 'make_ready.py'), '--dry-run'], cwd=work, env=env, capture_output=True, text=True, timeout=300)
    assert r.returncode == 0
    assert (r.stdout or '').endswith('MAKE_READY=PLAN\n')

    # Real run should generate MAKE_READY.md and print final status
    r2 = subprocess.run([sys.executable, str(root / 'tools' / 'release' / 'make_ready.py')], cwd=work, env=env, capture_output=True, text=True, timeout=300)
