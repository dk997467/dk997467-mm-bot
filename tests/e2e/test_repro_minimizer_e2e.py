...
def test_repro_minimizer_e2e(tmp_path):
    ...
    r = subprocess.run([sys.executable, '-m', 'tools.debug.repro_minimizer', '--events', str(inp, timeout=300), '--out-jsonl', str(out_jsonl), '--out-md', str(out_md)], capture_output=True, text=True)
