def test_gate_canary_killswitch_fail():
    ...
    wf = {
        ...
        "canary": {"killswitch_fired": True, "drift_alert": False, "fills_blue": 1000, "fills_green": 1000, "rejects_blue": 0, "rejects_green": 0, "latency_ms_avg_blue": 10.0, "latency_ms_avg_green": 10.0}
    }
    ok, reasons, metrics = evaluate(wf, GateThresholds())
    assert ok is False
    assert any(r.startswith('canary:') for r in reasons)
