...
def test_killswitch_freeze_action():
    ...
    srv.config = SimpleNamespace(rollout=SimpleNamespace(traffic_split_pct=25, active='blue', salt='s', pinned_cids_green=[]), rollout_ramp=SimpleNamespace(enabled=True, steps_pct=[0,25,50], cooldown_after_rollback_sec=0), killswitch=SimpleNamespace(enabled=True, dry_run=False, max_reject_delta=0.01, max_latency_delta_ms=10, min_fills=10, action='freeze'))
    class M:
        def __init__(self):
            self._snap = {
    ...
    assert srv._ramp_state['frozen'] is True
    assert 'freeze' in srv.metrics.triggers
