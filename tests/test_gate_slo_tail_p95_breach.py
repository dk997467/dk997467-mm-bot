...
def test_slo_tail_p95_breach_triggers_last_reason():
    # tighten slo caps for test
    CANARY_GATE['slo_tail_min_sample'] = 50
    CANARY_GATE['slo_tail_p95_cap_ms'] = 30
    ...
    wf = {"symbol":"BTCUSDT","metadata":{"created_at_utc":"1970-01-01T00:00:00Z"},"canary":{
        ...
        "latency_ms_p95_blue":20.0,"latency_ms_p95_green":40.0,
        "latency_ms_p99_blue":80.0,"latency_ms_p99_green":90.0,
        "latency_samples_blue":200,"latency_samples_green":200
    }}
    ok, reasons, _ = evaluate(wf, thr)
    assert ok is False
    assert reasons[-1].endswith('slo_tail_p95_breach')
